...
class Player(BasePlayer):
    ...
    # Number of rules completed (i.e. 6 correct choices for rule)
...
def generate_decks(test_card: dict, layout):
    ...
    for letter in layout:
        if letter == 'c':
            card = dict(color=test_card['color'],
                        shape=ss.pop(), number=ns.pop())
        elif letter == 's':
            card = dict(shape=test_card['shape'],
                        color=cs.pop(), number=ns.pop())
        elif letter == 'n':
            card = dict(number=test_card['number'],
                        shape=ss.pop(), color=cs.pop())
        else:
...
def live_method(player: Player, data):
    ...
    if 'deck_number' in data:
        ...
        response_time_base = {} if player.response_time == "" else json.loads(
            player.response_time)
        response_time_my_list = [
        ] if not player.num_trials in response_time_base else response_time_base[player.num_trials]
        response_time_my_list.append(json.dumps({
            # "id": player.participant.id_in_group,
            "trial_number": player.num_trials,
            "rule": player.rule,
            "decision": player.layout[deck],
            "correct": is_correct,
...
class Results(Page):
    ...
    @staticmethod
    def vars_for_template(player: Player):
        reward = cu(player.num_correct * SESSION_CONFIG_DEFAULTS['real_world_currency_per_point'])
        return dict(
